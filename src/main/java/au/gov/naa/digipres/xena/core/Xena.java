...
public class Xena {
	...
	private static final String DEFAULT_PREFERENCES_FILE = "/default-preferences.properties";
	/*
	...
	public static String getVersion() {
		try {
			InputStream s = Xena.class.getResourceAsStream(DEFAULT_PREFERENCES_FILE);
			Properties p = new Properties();
			p.load(s);
			String version = p.getProperty("application.version");
			return version == null ? "unknown" : version;
		} catch (NullPointerException npe) {
			return "unknown - missing property \"application.version\"";
		} catch (IOException ioe) {
			return "unknown - ioexception on properties file";
		}
	}

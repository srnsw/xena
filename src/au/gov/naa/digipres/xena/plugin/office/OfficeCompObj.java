...
public class OfficeCompObj {
	...
	public OfficeCompObj(InputStream is) throws IOException {
		...
		length = readInt(is);
		m_applicationName = (length > 0 && length < MAX_INFO_STRING_LENGTH) ? new String(readByteArray(is, length), 0, length - 1) : "";
		...
		if (m_applicationName != null && m_applicationName.equals("Microsoft Project 4.0") == true) {
			...
			length = readInt(is);
			m_fileFormat = (length > 0 && length < MAX_INFO_STRING_LENGTH) ? new String(readByteArray(is, length), 0, length - 1) : "";
			length = readInt(is);
			m_applicationID = (length > 0 && length < MAX_INFO_STRING_LENGTH) ? new String(readByteArray(is, length), 0, length - 1) : "";
		}
	}

...
public abstract class ArchiveNormaliser extends AbstractNormaliser
{
	...
	public void parse(InputSource input, NormaliserResults results) throws SAXException, java.io.IOException
	{
		...
		try
		{
			...
			{ 
                ...
                atts.addAttribute(ARCHIVE_URI, ENTRY_ORIGINAL_PATH_ATTRIBUTE, 
                                  ENTRY_ORIGINAL_PATH_ATTRIBUTE, "CDATA", entry.getName());
                               
                // Make sure date is formatted correctly
                SimpleDateFormat formatter = new SimpleDateFormat(DATE_FORMAT_STRING);                
                atts.addAttribute(ARCHIVE_URI, ENTRY_ORIGINAL_FILE_DATE_ATTRIBUTE, 
                                  ENTRY_ORIGINAL_FILE_DATE_ATTRIBUTE, "CDATA", formatter.format(entry.getOriginalFileDate()));
                                
                atts.addAttribute(ARCHIVE_URI, ENTRY_ORIGINAL_SIZE_ATTRIBUTE, 
                                  ENTRY_ORIGINAL_SIZE_ATTRIBUTE, "CDATA", String.valueOf(entry.getOriginalSize()));
                atts.addAttribute(ARCHIVE_URI, ENTRY_OUTPUT_FILENAME, 
                                  ENTRY_OUTPUT_FILENAME, "CDATA", entryOutputFilename);
	...
	private NormaliserResults normaliseArchiveEntry(XenaInputSource childXis, AbstractNormaliser entryNormaliser,  
									   				File entryOutputFile, OutputStream entryOutputStream,
									   				FileNamerManager fileNamerManager, Type fileType) 

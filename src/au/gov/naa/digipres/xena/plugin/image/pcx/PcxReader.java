 ...
 *
 * PcxReader, Version 1.10 [05/02/2003]
 * Copyright (c) 2000-2003 by Matthias Burg
 * All rights reserved
 ...
 *   - PCX Files with 1 Bit black & white Colors (since Version 1.1)
 *   - PCX Version 3.0 with 8 Bit (=256) Colors (since Version 1.0)
 *   - PCX Version 3.0 with 24 Bit (=16.7 Mio) Colors (since Version 1.0)
 *
...
{
	/*
	 * This is the main-class of the PcxReader. It reads the PCX-Data from a
	 * stream and converts it into an Image-Class.
	 */
	...
	public static Image decodeImage(InputStream in) throws IOException
	{
		...
		{
			/*
			 * The width of an PCX-Image must be even. That is why the width
			 * is increased when it was odd before.
			 */
			pcxwidth++;
		}
...
//		                   ", scanLineLength: " + scanLineLength);
		...
		{
			/*
			 * If the PCX-file has 256 colors there is a color-palete at the
			 * end of the file. This is 768b bytes long and contains the
			 * red- green- and blue-values of the colors.
			 */
		...
		{
			/*
			 * This is a new feature in Version 1.1 (May 2003) Now the PCX
			 * Reader is also able to read b&w images.
			 */
			...
			for (int i = 0; i < imagePixels; i++)
			{
				...
				while (k > 0)
				{
					...
					if (++width == pcxwidth)
					{
						...
						/*
						 * PCX lines must terminate on bit 16 Only whole
						 * bytes can be read
						 */
						if (i % 2 == 0)
						{
		...
		{
			throw new IOException("PCX Images with " + colour_planes + " colour planes and " + 
			                      bits_per_pixel + "bits per pixel are not currently supported");
		}
	...
	private static int decodeScanLine(int[] scanLineBuffer, InputStream in) throws IOException
	{
	    ...
	    do
	    {
	        ...
	        **
	        ** Read a byte of data.  If the two MSBs are 1 then this byte

...
public class PluginManager {
	 ...
	 */
	public static String getPluginName(File pluginFile) throws IOException, MalformedURLException {
	...
	public void loadPlugins(Collection<String> plugins) throws IOException, XenaException {
		...
		/*
		 * The following code snippet simply lists out each of the plugins that were not able to loaded for some reason.
		 */
	...
	private void initPlugin(String name, ClassLoader cl) throws XenaException, IOException {
		...
		JarPreferences pp = root.node(name, cl);
		initPlugin(name, pp);
	}
	 ...
	 */
	private void initPlugin(String name, JarPreferences preferences) throws XenaException, IOException {
		...
		while (dependancyGraph.size() != 0) {
			...
			while (it.hasNext()) {
				...
				while (dependancyIterator.hasNext()) {
					...
					if (!dependancyGraph.containsKey(dependancy) && !pluginPlan.contains(dependancy) && !"all".equals(dependancy)

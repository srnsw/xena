 ...
 *
 */
public abstract class ODFGuesser extends DefaultGuesser {

	public static final byte[][] ZIP_MAGIC = {{0x50, 0x4B, 0x03, 0x04}};
	private static final String MIMETYPE_FILENAME = "mimetype";
	...
	public Guess guess(XenaInputSource source) throws IOException {
		...
		if (guess.getMagicNumber() == GuessIndicator.TRUE) {
			ZipInputStream zipStream = new ZipInputStream(source.getByteStream());

			ZipEntry zipEntry = zipStream.getNextEntry();
			boolean found = false;
			while (zipEntry != null) {
				// Look for the file inside the zip called 'mimetype'
				if (zipEntry.getName().equals(MIMETYPE_FILENAME)) {

					// Read the first line of the mimetype file - should be the mimetype of the ODF document.
					BufferedReader zipEntryReader = new BufferedReader(new InputStreamReader(zipStream));
					String mimetype = zipEntryReader.readLine();

					if (stringInArr(mimetype, getIdentifyingMimetypes())) {
						found = true;
						break;
					}
				}
	...
	protected abstract String[] getIdentifyingMimetypes();

...
public class SimpleViewer {
	...
	public SimpleViewer() {
        ...
        for(int ii=0;ii<seps.length;ii++){
            int id=file.lastIndexOf(seps[ii]);
            if(id!=-1)
                file=file.substring(id+1,file.length());
        }
        System.out.println("file="+file);
        
		setupViewer();
        openDefaultFile("jar:/org/jpedal/file.pdf");
	} /**/


    /**
	 * setup and run client, loading defaultFile on startup
	 * @deprecated - use openDefaultFile(defaultFile)
	 */
...
//	        }
	...
	public void setupViewer() {
		...
		boolean showFirstTimePopup = propValue.length()>0 && propValue.equals("true");
		...
		if(properties != null && (propValue.length()>0 && propValue.equals("true"))){
	        ...
	        JOptionPane.showMessageDialog(decode_pdf, Messages.getMessage("Beta release Javascript support\n" +
	        "GPL version does not contain Javascript support - please look at full version"));
...
//		decode_pdf.setTransferHandler(dth);
		...
		if(!wasUpdateAvailable && propValue.length()>0 && propValue.equals("true")){
	 ...
	 * 
	 */
	public void executeCommand(int commandID, Object[] args){
		currentCommands.executeCommand(commandID, args);
	}

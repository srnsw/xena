		...
		if (qName.equalsIgnoreCase(MultiPageNormaliser.MULTIPAGE_PREFIX + ":" + MultiPageNormaliser.PAGE_TAG)) {
			...
			if (multipageLevel == 0) {
				...
				try {
					...
					// Export file
					ExportResult pageExportResult = normaliserManager.export(xis, outputDirectory, pageOutputFilename, true, deNormaliser);
					childExportResultList.add(pageExportResult);
					pageIndex++;

...
public class XOMHandler implements ContentHandler, LexicalHandler, DeclHandler, DTDHandler {
	...
	public XOMHandler(NodeFactory factory) {
		this.factory = factory;

		// JRW - initialisation moved here
	...
	public void startDocument() {
		...
		inDTD = false;
		// JRW - need to instantiate document, parents, parent and current in the constructor as we will not be calling start document
		//        document = factory.startMakingDocument();
		//		parents = new ArrayList();
		//		parent = document;
		//		current = document;
		...
		if (element != null) { // wasn't filtered out
			...
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				if (qName.startsWith("xmlns:") || qName.equals("xmlns")) {
					...
					Nodes nodes = factory.makeAttribute(qName, namespace, value, convertStringToType(attributes.getType(i)));
	...
	protected boolean inInternalSubset() {
		...
		if (!usingCrimson && !inExternalSubset) {
			return true;
		}
		String currentURI = locator.getSystemId();
		if (currentURI == documentBaseURI) {
			return true;
		}
		if (currentURI.equals(documentBaseURI)) {
			return true;
		}
		return false;
